...
void ArraySD::Initialize(G4HCofThisEvent*)
{
...
{ 
  ...
  G4double tofHit = aStep->GetDeltaTime(); // time difference/tof of step-----------HERE
  G4double tHitStart =  aStep->GetPreStepPoint()->GetGlobalTime();//-=============HERE
  G4double tHitEnd =  aStep->GetPostStepPoint()->GetGlobalTime();//==============HERE
  G4int Hitptype =  aTrack->GetDynamicParticle()->GetPDGcode();//==============HERE
  ...
  if (fHitID[id]==-1){
    // Its a new hit...make one, and store start paramaters   
    xHit = new ArrayHit;
    xHit->SetPos(hpos);
    xHit->SetID(id);
    xHit->SetTime(tHit); // this is global time of first hit/step?!?!?!
    // xHit->SetTOF(tofHit);
    //    Hit->SetPol(*PolNN);
    fHitID[id] = fCollection->insert(xHit) -1;
    fHits[fNhits++]=id;
  }
  else xHit =  (*fCollection)[fHitID[id]];
  // do following regardless of new or used hit buffer
  xHit->SetTOF(tofHit); // add tof of step =========================HERE
  xHit->SetTStart(tHitStart); // add start time of step =========================HERE
  xHit->SetTEnd(tHitEnd); // add end time of step =========================HERE
  xHit->SetPtype(Hitptype); // add end time of step =========================HERE
  xHit->AddEnergy(edep);
  xHit->AddEnergyB(mb);
  xHit->SetDedx(dedxM);  // save mean dedx
  if( (id == 1)||(id == 6) ){
    G4ThreeVector hpb = aStep->GetPostStepPoint()->GetPosition();
    if(!xHit->AddStep(trID,parID,stepID,edep,&p4,&hpos,&hpb,tofHit,tHitStart,tHitEnd))//===============HERE
...
void ArraySD::EndOfEvent(G4HCofThisEvent* HCE)
{
  /*
  ArrayHit* xHit;
