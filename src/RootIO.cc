...
void RootIO::SetOutput(const char* fn)
{
  ...
  fArr_tof=new Float_t[fArrayTot]; //====================HERE
  fArr_tstart=new Float_t[fArrayTot]; //====================HERE
  fArr_tend=new Float_t[fArrayTot]; //====================HERE
  fArr_ptype=new Int_t[fArrayTot]; //====================HERE
  ...
  fStTOF= new Float_t[fStepTot];//=============HERE
  fStTstart= new Float_t[fStepTot];//=============HERE
  fStTend= new Float_t[fStepTot];//=============HERE
  Int_t basket =64000;
  ...
  fTreeO->Branch("arrTOF",fArr_tof,"fArr_tof[fNArr]/F",basket);//=================HERE
  fTreeO->Branch("arr_tstart",fArr_tstart,"fArr_tstart[fNArr]/F",basket);//==========HERE
  fTreeO->Branch("arr_tend",fArr_tend,"fArr_tend[fNArr]/F",basket);//==========HERE
  fTreeO->Branch("arr_ptype",fArr_ptype,"fArr_ptype[fNArr]/I",basket);//==========HERE
  ...
  fTreeO->Branch("StTOF",fStTOF,"fStTOF[fNSt]/F",basket);//=================HERE
  fTreeO->Branch("StTstart",fStTstart,"fStTstart[fNSt]/F",basket);//=================HERE
  fTreeO->Branch("StTend",fStTend,"fStTend[fNSt]/F",basket);//=================HERE
}
//
//---------------------------------------------------------------------------
void RootIO::WriteHit(G4HCofThisEvent* HitsColl)
{
  ...
  for(G4int i=0;i<CollSize;i++){
    ...
    while( !( hc = (ArrayHitsCollection*)(HitsColl->GetHC(hci++)) ) ) ;
    //if(!hc)continue; //no hits in that detector
    G4int hc_nhits=hc->entries();
    if(hc->GetName()=="ArraySDHitsArraySD"){
      fNArr=hc_nhits;
      for(Int_t j=0; j<fNArr; j++){
	ArrayHit* hit = (ArrayHit*)(hc->GetHit(j));
	fArr_e[j]=hit->GetEdep();
	fArr_ew[j]=hit->GetEdepB();
	fArr_t[j]=hit->GetTime();
	fArr_tof[j]=hit->GetTOF();//=========================HERE
	fArr_tstart[j]=hit->GetTStart();//=========================HERE
	fArr_tend[j]=hit->GetTEnd();//=========================HERE
	fArr_ptype[j]=hit->GetPtype();//=========================HERE
	fArr_te[j]=hit->GetTimeE();
	fArr_x[j]=hit->GetPos().x();
	fArr_y[j]=hit->GetPos().y();
	fArr_z[j]=hit->GetPos().z();
	fArr_i[j]=hit->GetID();
	nst = hit->GetNSt();
	...
	for(Int_t k=0; k<nst; k++){
	  ...
	  fStTOF[k1] = hit->GetHitStep(k)->TOF;//================================HERE
	  fStTstart[k1] = hit->GetHitStep(k)->TStart;//==============================HERE
	  fStTend[k1] = hit->GetHitStep(k)->TEnd;//==============================HERE
	}
	fNSt += nst;
      }
    }
